...
const staffRoles = ['STORE_OPERATOR'] as const;
...
export async function userRoutes(app: FastifyInstance, database: Database, env: AppEnv): Promise<void> {
      ...
      const result = await client.query(
        `SELECT u.id, u.name, u.email, u.status AS "accountStatus", membership.role,
         ...
         WHERE membership.tenant_id = $1 AND membership.role <> 'COURIER'
           AND EXISTS (SELECT 1 FROM user_store_access scoped WHERE scoped.tenant_user_id=membership.id)
  ...
  // Password recovery is now global, email-verified and self-service.
  app.get('/users/password-reset-requests', {
    preHandler: [auth, requireRoles('TENANT_MANAGER')],
  }, async () => ({ data: [] }));
  app.post('/users/:userId/password', {
    preHandler: [auth, requireRoles('TENANT_MANAGER')],
  }, async () => { throw forbidden('A própria pessoa deve usar a recuperação por e-mail na tela de login.'); });
  ...
  app.patch('/users/:userId', { preHandler: [auth, requireRoles('TENANT_MANAGER')] }, async (request) => {
    ...
    return withTenantTransaction(database, request.auth, async (client) => {
      ...
      if (target.role === 'TENANT_MANAGER') throw forbidden('Somente o Master altera gestores.');
      const manageable=await client.query<{allowed:boolean}>('SELECT rastreia.can_manage_scoped_person($1) AS allowed',[userId]);
      if(!manageable.rows[0]?.allowed)throw forbidden('Esta pessoa possui vínculos fora da unidade selecionada. Solicite ao Master.');
